阿里云 RDS 通过代理组件实现读写分离 Terraform 模块

# terraform-alicloud-rds-read-write-splitting

[English](https://github.com/alibabacloud-automation/terraform-alicloud-rds-read-write-splitting/blob/main/README.md) | 简体中文

该 Terraform 模块在阿里云上创建完整的 RDS 读写分离解决方案，使用数据库代理组件实现读写分离。该模块实现了[RDS 通过代理组件实现读写分离](https://www.aliyun.com/solution/tech-solution/read-write-splitting-through-rds-proxy)解决方案，涉及专有网络（VPC）、交换机（VSwitch）、云服务器（ECS）、云数据库 RDS MySQL 及只读实例、数据库代理等资源的创建和部署，实现自动读写分离功能。

## 使用方法

该模块创建完整的 RDS 读写分离基础设施，包括 VPC、ECS 实例、RDS 数据库及只读副本、数据库代理配置。
...
* [完整示例](https://github.com/alibabacloud-automation/terraform-alicloud-rds-read-write-splitting/tree/main/examples/complete)
